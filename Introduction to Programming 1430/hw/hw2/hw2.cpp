...
int main(int argc, char* argv[])
{
    ...
    string direction, commandline, commdiff, commname, type, remove, removetype;
    string pos, commid;
    while (!commandFile.eof()) {
        ...
        if (commandline == "sort") {
            ...
            getline(commandFile, commid, ',');
            while (commid.length() == 0) {
                commandFile >> commid;
            }
            getline(commandFile, line, ':');
            while (line.length() == 0) {
            ...
            while (commdiff.length() == 0) {
                getline(commandFile, commdiff, '\n');
            }
            int commid1 = stoi(commid);
            int pos1 = stoi(pos);
            Llist.add(commid1, commname, commdiff, pos1 - 1);
        }
        else if (commandline == "remove") {
            getline(commandFile, removetype, ':');
            while (removetype.length() == 0) {
                getline(commandFile, removetype, ':');
            }
            commandFile >> remove;
